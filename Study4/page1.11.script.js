document.addEventListener('DOMContentLoaded', () => {
  ...
  soundsMenu.addEventListener('change', (event) => {
    ...
    logMessage("Change event in soundsMenu from: event.key " + event.key);
  ...
  soundsMenu.addEventListener('keydown', (event) => {
    ...
    logMessage("Change event in soundsMenu from keypad: event.key " + event.key);
    if (event.key === 'Enter') {
        soundChange();
    }
  });

  soundsMenu.addEventListener('touchstart', (event) => {
    console.log("Change event in soundsMenu from ouchstart: event.key " + event.key);
    logMessage("Change event in soundsMenu from ouchstart: event.key " + event.key);
    if (event.key === 'Enter') {
        soundChange();
    }
  });
 ...
 soundsMenu.addEventListener('touchmove', (e) => {
  ...
  soundsMenu.addEventListener('touchend', (e) => {
    if (!isScrolling) {
      // Treat as tap
      ...
      console.log('Scroll detected, no selection triggered');
    }
  });
